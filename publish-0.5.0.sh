#!/usr/bin/env bash
# Publish laserbrain 0.5.0 — the whole toolkit, on the public surface.
#
# 0.5.0 exports what was already in the package but unreachable: the field (read and
...
checks = {
    "version is 0.5.0":        "__version__ = '0.5.0'" in body,
...
sys.exit(0 if all(checks.values()) else '  sdist does not carry what 0.5.0 claims — do not publish')
PY
