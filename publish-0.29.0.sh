#!/usr/bin/env bash
# Publish laserbrain 0.29.0 to PyPI.
#
...
# refuses to upload if 0.29.0 already exists.
#
...
# 3.5 · IS THE WHEEL NEWER THAN THE SOURCE?
#
# Added after 0.26.0 shipped stale. That wheel was built at 19:06, mcp.py changed at 19:10
# and grammar.json at 19:13, and the upload went out before the rebuild — so the published
# package carried grammar 1.18.0 and no modulate while the tree had both. PyPI versions
# cannot be overwritten, so it cost a version number.
#
# Every other check here reads the TREE or the installed wheel, and both were internally
# consistent. Nothing compared their AGES. Third time a wheel has shipped behind its source
# (0.9.0, 0.12.0, 0.26.0), and it is one stat call.
STALE=$(DIST_DIR=dist_0290 python3 - <<'PYEOF'
import glob, os
whl = sorted(glob.glob(os.environ['DIST_DIR'] + '/*.whl'))
if not whl:
    print('NO WHEEL'); raise SystemExit
wt = os.path.getmtime(whl[-1])
print(' '.join(f for f in glob.glob('laserbrain/*.py') + glob.glob('laserbrain/*.json')
               if os.path.getmtime(f) > wt))
PYEOF
)
if [ -n "$STALE" ]; then
  echo "  the wheel is OLDER than: $STALE"
  echo "  rebuild before publishing — this is how 0.26.0 shipped without modulate."
  exit 1
fi
echo "  ok  wheel is newer than every source file"
